...
export function SectionLibrary({
  ...
  const heroStats = React.useMemo(() => {
    ...
    return {
      ...
      byCategory: HERO_FILTER_OPTIONS.slice(1).map((option) => ({
        label: option.label,
          ...
          <div className="p-6 border-b border-gray-200 space-y-4">
            ...
            {selectedCategory === SECTION_CATEGORIES.HERO && (
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {HERO_FILTER_OPTIONS.map((option) => {
                    const categoryCount =
                      option.value === "all"
                        ? heroStats.total
                        : heroStats.byCategory.find(
                            (cat) => cat.label === option.label,
                          )?.count || 0;
                ...
                <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-100">
                  <div className="text-xs font-medium text-blue-900 mb-2">
                    ✨ All 10 Hero Variants Available
                  </div>
          ...
          <div className="p-6 border-t border-gray-200 bg-gray-50">
            <div className="flex items-center justify-between text-sm text-gray-500">
              <span>
                {selectedCategory === SECTION_CATEGORIES.HERO ? (
                  <>
                    Showing {sections.length} of {getHeroSections().length} hero
                    variants
                  </>
                ) : (
                  <>
                    Showing {sections.length} of {getActiveSections().length}{" "}
                    available sections
                  </>
                )}
              </span>
