...
export function WorkflowIntegrationTest() {
  ...
  const runWorkflowTests = async () => {
    ...
    try {
      ...
      for (const heroSection of heroSections.slice(0, 3)) {
      ...
      const selectionSuccess = selectionTests.every(
        (test) =>
      ...
      results.push({
        ...
        details: {
          ...
          successfulSelections: selectionTests.filter(
            (t) => t.hasConfig && t.hasEditor && t.hasPreview && t.isActive,
          ).length,
          selectionTests,
        },
      });
      ...
      const editorsLoaded = editorLoadingTests.filter(
      ...
      const previewsLoaded = previewLoadingTests.filter(
        (result) => result.status === "fulfilled",
      ).length;
      ...
      const componentsLoaded = componentLoadingTests.filter(
        (result) => result.status === "fulfilled",
      ).length;
        ...
        details: {
          ...
          completionPercentage:
            (Object.values(workflowSteps).filter(Boolean).length /
              Object.keys(workflowSteps).length) *
            100,
        },
      });
      ...
      try {
        await HeroSectionFactory.loadComponent(
          "invalid-variant" as HeroVariant,
        );
      ...
      const errorHandlingSuccess = errorHandlingTests.every(
        (test) => test.handled,
      );
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          End-to-end testing of the complete hero sections workflow from
          selection to rendering
        </p>
      </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          {testResults.length > 0 ? (
            <div className="space-y-2">
              ...
              >
                {passedTests}/{totalTests} Steps Passed (
